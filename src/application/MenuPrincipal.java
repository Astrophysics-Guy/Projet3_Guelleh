...
public class MenuPrincipal extends Application {
		...
		try {
			 ...
			 * String strTypeAuto = strAuto.substring(0, 4), strOrientation = strAuto.substring(5, 6), strCouleur = strAuto.substring(7, strAuto.length() - 4);
				...
				if (strAuto.substring(5, 6).equals("H")) 
					System.out.println(files[i].getName() + " " + strAuto.substring(7, strAuto.length() - 4));
			 ...
			 * if (strAuto.substring(7, 8).equals("H")) 
					System.out.println(files[i].getName() + " " + strAuto.substring(9, strAuto.length() - 4));
				else 
					System.out.println(files[i].getName() + " " + strAuto.substring(9, strAuto.length() - 4));
			...
			for (int i = 0; i < files.length; i++) if (files[i].isFile()) {
				if (files[i].getName().substring(0, 4).equals("auto")) 
					arrImageVoitures.add(new ImageVoiture(files[i].getName(), // Nom fichier
													files[i].getName().substring(0, 4), // Type d'auto
													files[i].getName().substring(5, 6), // Orientation
													files[i].getName().substring(7, files[i].getName().length() - 4))); // Couleur
				
				
				else if (files[i].getName().substring(0, 6).equals("camion")) 
					arrImageVoitures.add(new ImageVoiture(files[i].getName(), // Nom fichier
													files[i].getName().substring(0, 6), // Type d'auto
													files[i].getName().substring(7, 8), // Orientation
													files[i].getName().substring(9, files[i].getName().length() - 4))); // Couleur
			}
			...
			for (int i = 0; i < strImagesLogoGrille.length; i++) 
				imageViewsLogoGrille[i] = new ImageView(new Image(strImagesLogoGrille[i].toString()));
			...
			for (int i = 0; i < strImages.length; i++) {
				imageViews[i] = new ImageView(new Image(strImages[i].toString()));
			...
			btnFacile.setFont(font(15));
			btnMoyen.setFont(font(15));
			btnDiff.setFont(font(15));
			...
			/*primaryStage.setOnCloseRequest(e -> { //////////////////////// NE PAS OUBLIER DE L'ACTIVÉ A LA FIN
				e.consume();
	...
	private class actionMouseEvent implements EventHandler<MouseEvent> {
		...
		public void handle(MouseEvent event) {
			...
			if (strID.equals("1")) { // Facile
				new MenuJeu(stage, 1, imageViewsLogoGrille, arrVoituresFacile, arrImageVoitures).show();
				
				stage.close();
			}
			else if (strID.equals("2")) { // Moyen
				new MenuJeu(stage, 2, imageViewsLogoGrille, arrVoituresMoyen, arrImageVoitures).show();
				
				stage.close();
			}
			else if (strID.equals("3")) { // Difficile
				new MenuJeu(stage, 3, imageViewsLogoGrille, arrVoituresDiff, arrImageVoitures).show();
